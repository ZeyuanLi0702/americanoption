...
class AmericanOptionSolver():
    def __init__(self, riskfree, dividend, volatility, strike, maturity, option_type):
        ...
        self.K = strike
        self.T = maturity
        self.collocation_num = 32#12
        self.quadrature_num = 32#24
        self.integration_num =65#2 * self.quadrature_num
        self.max_iters = 8#200
        self.iter_tol = 1e-5
        self.shared_B0 = []
        self.shared_B = []
        self.shared_B_old = []
    ...
    def assign_par(self, l, m, n, p):

        self.collocation_num = n
        self.max_iters = m
        self.integration_num = l
        self.quadrature_num = p
    ...
    def D_func(self, tau, B):
        ...
        K12 = self.K12(tau, B)
        if self.option_type == qd.OptionType.Put:
            return self.PDF_dplus(tau, B / self.K) / (self.sigma * np.sqrt(tau)) + self.CDF_pos_dplus(tau,
            B / self.K) + self.q * (K12)
        else:
            return self.PDF_neg_dplus(tau, B / self.K) / (self.sigma * np.sqrt(tau)) - self.CDF_neg_dplus(tau,
             B / self.K) + self.q * (K12)
    ...
    def Nprime_func(self, tau, Q):
        tau = max(1e-10, tau)
        if self.option_type == qd.OptionType.Put:
            return -self.dminus(tau, Q / self.K) * self.PDF_dminus(tau, Q / self.K) / (
                        Q * self.sigma * self.sigma * tau) \
                - self.r * self.quadrature_sum(self.Nprime_integrand, tau, Q, self.quadrature_num)
        else:
            return -self.dminus(tau, Q / self.K) * self.PDF_neg_dminus(tau, Q / self.K) / (
                        Q * self.sigma * self.sigma * tau) \
                - self.r * self.quadrature_sum(self.Nprime_integrand, tau, Q, self.quadrature_num)
